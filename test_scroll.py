...
class MyApp(tk.Tk):
    def __init__(self, title="Sample App", *args, **kwargs):
        ...
        master_frame = tk.Frame(self, bg="Light Green", bd=3, relief=tk.RIDGE)
        ...
        frame1a = tk.Frame(top_frame, bg="Green", bd=2, relief=tk.GROOVE)
        frame1a.grid(row=0, column=0, sticky=tk.NSEW)
        frame1a.columnconfigure(0, weight=1)
        #frame1.rowconfigure((0,1), weight=1)

        frame1b = tk.Frame(top_frame, bg="Blue", bd=2, relief=tk.GROOVE)
        frame1b.grid(row=0, column=1, sticky=tk.NSEW)
        frame1b.columnconfigure(0, weight=1)
        #frame1b.rowconfigure((0,1), weight=1)

        frame1c = tk.Frame(top_frame, bg="Red", bd=2, relief=tk.GROOVE)
        frame1c.grid(row=0, column=2, sticky=tk.NSEW)
        frame1c.columnconfigure(0, weight=1)
        #frame1c.rowconfigure((0,1), weight=1)

        label1a = tk.Label(frame1a, text="Frame1 Contents", bg=LABEL_BG)
        label1a.grid(row=0, column=0, pady=5, sticky=tk.N)
        #label1.columnconfigure(0, weight=1)

        label1b = tk.Label(frame1b, text="Frame1b Contents", bg=LABEL_BG)
        label1b.grid(row=0, column=0, pady=5, sticky=tk.N)#tk.NSEW)
        #label1b.columnconfigure(0, weight=1)

        label1c = tk.Label(frame1c, text="Frame1c Contents", bg=LABEL_BG)
        label1c.grid(row=0, column=0, pady=5, sticky=tk.N)
        #label1c.columnconfigure(0, weight=1)

        cb_var1a = tk.IntVar()
        checkbutton1a = tk.Checkbutton(frame1a, text="StartCheckBox", variable=cb_var1a)
        checkbutton1a.grid(row=1, column=0, padx=2, sticky=tk.N)
        # checkbutton1.columnconfigure(0, weight=1)

        cb_var1b = tk.IntVar()
        checkbutton1b = tk.Checkbutton(frame1b, text="Startbb", variable=cb_var1b)
        checkbutton1b.grid(row=1, column=0, padx=2, sticky=tk.N)
        # checkbutton1b.columnconfigure(0, weight=1)

        cb_var1c = tk.IntVar()
        checkbutton1c = tk.Checkbutton(frame1c, text="Startccc", variable=cb_var1c)
        checkbutton1c.grid(row=1, column=0, padx=2, sticky=tk.N)
        # checkbutton1c.columnconfigure(0, weight=1)
        ...
        # label2 = tk.Label(frame2, text="Frame2 Contents", bg=LABEL_BG)
        # label2.grid(row=0, column=0, pady=5, sticky=tk.NW)

        # Add a canvas in that frame.
        #Ten canvas dziala i jest responsive
        ...
        labels_frame = tk.Frame(canvas, bg="Red", bd=2)
        ...
        for i in range(15):
            for j in range(3):
                label = tk.Label(master=labels_frame, text=f"Row {i} Column {j}")
